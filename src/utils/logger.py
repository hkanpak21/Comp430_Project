...
def setup_logger(log_dir, log_filename, args):
    ...
    if not file_exists:
        header = ["timestamp"] + list(vars(args).keys()) + ["round", "train_loss", "accuracy", "epsilon_per_round", "total_epsilon", "current_sigma", "current_C"]
...
def log_results(log_filepath, args, round_num, train_loss, accuracy, epsilon_per_round, total_epsilon, current_sigma=None, current_C=None):
    ...
    log_entry = [timestamp] + list(vars(args).values()) + [
        ...
        f"{current_sigma:.4f}" if current_sigma is not None else "N/A",
        f"{current_C:.4f}" if current_C is not None else "N/A"
    ]
    try:
